...
@section('header', 'Edit Data Parameter')

@section('content')
<div class="stat-card" style="max-width: 800px; margin: 0 auto;">
    <form action="{{ route('admin.parameters.update', $parameter) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <input type="text" name="kode_param" value="{{ old('kode_param', $parameter->kode_param) }}" required
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <input type="text" name="nama_param" value="{{ old('nama_param', $parameter->nama_param) }}" required
                   style="width: 100%; padding: 0.75rem; border: 1.5px solid #e5e7eb; border-radius: 10px; font-size: 0.85rem; outline: none;">
            @error('nama_param')
                <p style="color: #ef4444; font-size: 0.7rem; margin-top: 0.25rem;">{{ $message }}</p>
            @enderror
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <select name="kategori" required
                ...
                <option value="kimia_darah" {{ old('kategori', $parameter->kategori) == 'kimia_darah' ? 'selected' : '' }}>Kimia Darah</option>
                <option value="urin" {{ old('kategori', $parameter->kategori) == 'urin' ? 'selected' : '' }}>Urinalisis</option>
                <option value="serologi" {{ old('kategori', $parameter->kategori) == 'serologi' ? 'selected' : '' }}>Serologi</option>
                <option value="narkoba" {{ old('kategori', $parameter->kategori) == 'narkoba' ? 'selected' : '' }}>Narkoba</option>
                <option value="lainnya" {{ old('kategori', $parameter->kategori) == 'lainnya' ? 'selected' : '' }}>Lainnya</option>
            </select>
            @error('kategori')
                <p style="color: #ef4444; font-size: 0.7rem; margin-top: 0.25rem;">{{ $message }}</p>
            @enderror
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <input type="text" name="satuan" value="{{ old('satuan', $parameter->satuan) }}"
                   style="width: 100%; padding: 0.75rem; border: 1.5px solid #e5e7eb; border-radius: 10px; font-size: 0.85rem; outline: none;"
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1.5rem;">
            <div>
                ...
                <input type="number" step="any" name="nilai_normal_min" value="{{ old('nilai_normal_min', $parameter->nilai_normal_min) }}"
                       style="width: 100%; padding: 0.75rem; border: 1.5px solid #e5e7eb; border-radius: 10px; font-size: 0.85rem; outline: none;">
            </div>
            <div>
                <label style="display: block; font-size: 0.8rem; font-weight: 500; color: #374151; margin-bottom: 0.5rem;">
                ...
                <input type="number" step="any" name="nilai_normal_max" value="{{ old('nilai_normal_max', $parameter->nilai_normal_max) }}"
                       style="width: 100%; padding: 0.75rem; border: 1.5px solid #e5e7eb; border-radius: 10px; font-size: 0.85rem; outline: none;">
            </div>
        </div>
        ...
        <div style="margin-bottom: 2rem;">
            <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $parameter->is_active) ? 'checked' : '' }}
